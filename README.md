...
## Application Features and How the Application Works
...
- Expect roughly 1–3 minutes per batch. After you pick a date range the panel shows a short stored/new count, then `Downloading x of y emails` if Gmail has to fetch, then which batch is being read.
...
- Past searches sit behind **View my past searches**, in a table of search date, question, and date range (newest first). Click a row to reopen the saved findings — it does not run the search again. Hover a truncated question to read all of it.
